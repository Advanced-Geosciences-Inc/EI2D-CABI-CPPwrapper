...
class EI2DWrapper:
    ...
    def forward_fd_simple(self, n_elec: int = 8, spacing: float = 1.0) -> dict:
        ...
        expected_gNumElemY = nNy - 1  # Should be 4 for 5 nodes  
        if p2[0] == expected_gNumElemX and q2[0] == expected_gNumElemY:
            print(f"   ✅ Parameter bounds correctly match element dimensions!")
        else:
            print(f"   ⚠️  Parameter bounds mismatch: expected {expected_gNumElemX}x{expected_gNumElemY}")
            
        # Also update the parameter dimensions for SetNumParamForward
        print(f"   Will call SetNumParamForward({nParamX}, {nParamY})")
...
class EI2DRealDataProcessor:
    ...
    def _run_real_forward_modeling(self, electrodes: List, measurements: List, 
        ...
        try:
            ...
            # CRITICAL FIX: Parameter arrays must use ELEMENT bounds, not NODE bounds
            ...
            p2 = np.array([nNx - 1], dtype=np.int32)  # CRITICAL: Element bounds = nodes - 1
            q1 = np.array([1], dtype=np.int32)
            q2 = np.array([nNy - 1], dtype=np.int32)  # CRITICAL: Element bounds = nodes - 1
        ...
        try:
            ...
            if self.lib:
                try:
                    ...
                    # Use keyword arguments to avoid signature issues
